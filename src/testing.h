...
count_tested++;\
if (!_st) { printf("Failed for token '%s': expected '%s'\n", lexer.peek().raw.c_str(), tok); break; }\
count_suc++;\
lexer.consume();}
...
_st = save == r;\
count_tested++;\
if (!_st) { printf("Failed for expression '%s': expected '%s'\n", save.c_str(), r); break; }}\
count_suc++;

#define ASSERT_EQ(l, r) { auto save = l; \
_st = l == r;\
count_tested++;\
if (!_st) { printf("Failed for expression '%s': expected '%s' found '%s'\n", #l, #r, std::to_string(save).c_str()); break; }\
count_suc++;}

#define ASSERT_TRUE(e) {_st = e; \
count_tested++;\
if (!_st) { printf("Failed for expression: expected '%s' to be true\n", #e); return false; }\
count_suc++;}
